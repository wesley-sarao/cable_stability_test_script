...
def get_temp_no_check():
    response1 = tc.query('?tcuve')
    prefix1, value1 = response1.split('=')
    response2 = tc.query('?tproduit')
    prefix2, value2 = response2.split('=')
...
def get_temperature(prev_value, max_retries=5, delay=1, threshold=5):
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            prefix1, value1 = response1.split('=')
            prefix2, value2 = response2.split('=')
            temperature1 = float(value1)
            temperature2 = float(value2)
                # Check if the new value differs too much from the previous value
            if prev_value is not None and abs(temperature1 - prev_value) > threshold:
               print(f"Anomalous temperature detected: {temperature1} (diff: {abs(temperature1 - prev_value)})")
            else:
                return temperature1, temperature2
        except (ValueError, AttributeError) as e:
            print(f"Error parsing response: {response1}, Error: {e}")
...
try:
    ...
    chamber_temp, chamber_temp_pt100 = get_temp_no_check()
